...
class AccountsApplicationService:
    ...
    def register_account(self, body: dict) -> Accounts:
        user: Users = self.__users_repository.find_by_user_id(body.get('userId'))
        if user is None:
            raise ce.UserNotFound

        account_already_exists: bool = self.__accounts_service.retrieve_existing_account_by_user_id(user.id)
    ...
    def retrieve_account(self, account_id: int) -> Accounts:
        account: Accounts = self.__accounts_repository.find_by_account_id(account_id)
        if account is None:
